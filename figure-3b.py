...
resp_max_amp = 50
resp = nk.rsp_simulate(**resp_kwargs)
resp = minmax_scale(resp, feature_range=(-resp_max_amp, resp_max_amp))

# Add LRLR signal to EOG
event_start = 7  # time into the main signal this will show up
...
# Add respiration rate increase in RESP
resp_event_amplitude = 50
resp_event_frequency = 2
resp_event_duration = duration - event_end
resp_event_time = np.arange(0, resp_event_duration, 1/frequency)
resp_event_signal = resp_event_amplitude * np.sin(2 * np.pi * resp_event_frequency * resp_event_time)
resp[end_index:] += resp_event_signal
...
cutoff = 0.5
b, a = butter(N=2, Wn=cutoff / (0.5 * frequency), btype="high", analog=False)
resp[end_index:] = filtfilt(b, a, resp[end_index:])
...
fig, axes = plt.subplots(
    ...
    gridspec_kw={"hspace": 0, "height_ratios": [1, 1, 1, 1.5]},
)
...
plt.savefig("../output/figure-3b.png", dpi=300)
plt.savefig("../output/figure-3b.svg", format="svg")
plt.savefig("../output/figure-3b.eps", format="eps")
plt.savefig("../output/figure-3b.pdf", format="pdf")
plt.close()
